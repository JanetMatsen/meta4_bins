	...
	if (in_features) {
		...
		if ( tag == "     CDS " || tag == "     rRNA" || tag == "     tRNA" ) {
			...
			if (substr($2, 1, 10) == "complement") {
				...
				if (!isnum(substr(range[3], 1, 1)))
					range[3] = substr(range[3], 2)
			...
			# prefill the next orf's locus_tag include the CDS from gene
			# doesn't include this field
			if (index($1, "/") == 1) {
				label = extract_label($1);
				data = extract_data($0);
				if (label == "locus_tag") {
					orf[cds+1, label] = data;
				}
			}
		}
	}
